...
def run_workflow(pkg_path, var_file, out_dir, out_prefix, build, cols_var, cols_input):
	...
	## show ClinVar query summary DF
	print('\nClinVar query summary:', results['data_summary_df'])

	## show ClinVar pathogenic variants
	cols_show = results['patho_var_df'].columns.tolist()[:9] + ['patho_cond.set', 'patho_cond.nuniq', 'patho_cond.%', '_.patho_ALL_cond', 'preferred_name', 'rsid']
	print('\n\n\n\nClinVar pathogenic variants:\n\n', results['patho_var_df'][cols_show].head())
	return 0
...
if __name__ == "__main__":
	...
	parser = argparse.ArgumentParser(description='Driver script for the ClinVar_workflow exploratory analyses')
	...
	## 3. exit
	print('\n\n\nclinvar_exploratory_analysis.py complete. Goodbye.\n\n')
	exit(0)
